...
export default function ExecuteRent() {
  ...
  const [retryFail, setRetryFail]         = useState(true)
        ...
        <div style={{ flex:1, overflowY:'auto', background: COLORS.bg }}>
          <div style={{ background: theme.headerGrad, paddingTop:'20px', paddingBottom:'24px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'4px 16px 16px' }}>
              <button onClick={() => setScreen('detail')} style={{ width:'32px', height:'32px', background:'transparent', border:'none', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', padding:0 }}>
          ...
          <div style={{ flex:1, overflowY:'auto', background: COLORS.bg }}>
            ...
            <div style={{ background: theme.headerGrad, paddingTop:'20px', paddingBottom:'0' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'4px 16px 12px' }}>
                <button onClick={closeDetail} style={{ width:'32px', height:'32px', background:'rgba(255,255,255,0.12)', border:'none', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', padding:0, flexShrink:0 }}>
                ...
                <button onClick={() => setShowExitModal(true)} style={{ width:'32px', height:'32px', background:'rgba(255,255,255,0.12)', border:'none', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', padding:0, flexShrink:0 }}>
            ...
            <div style={{ padding:'16px 16px 36px', display:'flex', flexDirection:'column', gap:'6px' }}>
              ...
              {/* 즉시 납부 버튼 */}
              {row.payable && (
                <button onClick={handleImmediatePay}
                  style={{
              ...
              <div style={{ background: COLORS.bgCard, borderRadius:'16px', boxShadow: SHADOWS.card, overflow:'hidden' }}>
                {[
                  ...
                  <div key={item.label} style={{ padding:'13px 16px', display:'flex', alignItems:'center', gap:'12px', borderBottom: i < arr.length-1 ? `1px solid ${COLORS.borderSoft}` : 'none' }}>
                    <div style={{ width:'36px', height:'36px', borderRadius:'12px', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'16px', background: item.bg }}>
                      {item.icon}
                    </div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:'13px', fontWeight:600, color: COLORS.t1, marginBottom:'2px' }}>{item.label}</div>
              ...
              <div style={{ background: COLORS.bgCard, borderRadius:'16px', boxShadow: SHADOWS.card, overflow:'hidden' }}>
                {[
                  { icon:'📄', bg:'#EFF6FF', label:'임대차 계약서', sub:'계약서 첨부 시 증빙 센터 자동 보관', on: hasLease, toggle: () => setHasLease(!hasLease) },
                  { icon:'🧾', bg:'#F0FDF4', label:'세금계산서', sub:'납부 후 임대인 세금계산서 자동 수집', on: hasTax, toggle: () => setHasTax(!hasTax) },
                  { icon:'🏧', bg:'#FFF7ED', label:'이체확인증 · 현금영수증', sub:'이체 완료 후 자동 발행 및 저장', on: hasReceipt, toggle: () => setHasReceipt(!hasReceipt) },
                ].map((ev, i, arr) => (
                ...
                <div style={{ padding:'10px 16px', background: COLORS.bgMuted, display:'flex', gap:'8px', alignItems:'center' }}>
                  ...
                  <div style={{ fontSize:'11px', color: COLORS.t3 }}>저장 위치: <span style={{ fontWeight:700 }}>통합 증빙 센터</span> → 임대료 폴더</div>
                </div>
              </div>
              ...
              <div style={{ background: COLORS.bgCard, borderRadius:'16px', boxShadow: SHADOWS.card, padding:'16px', display:'flex', flexDirection:'column', gap:'14px' }}>
                {[
                  ...
                  { icon:'📅', label:'계약 만료 예정 알림', sub:'만료 30일 전 발송',       val:notifExpiry, set:setNotifExpiry },
                ].map((row, i) => (
                  <div key={i}>
