...
export default function ExecuteInsurancePremium() {
  ...
  const [editNotifExpiry, setEditNotifExpiry] = useState(true)
  ...
  const [addNotifExpiry, setAddNotifExpiry] = useState(true)
  ...
  function openDetail(id) {
    ...
    setEditNotifFail(it.notifFail); setEditNotifExpiry(it.notifExpiry)
    setSaved(false)
    setScreen('detail')
  }
  ...
  function handleAdd() {
    setItems(prev => [...prev, {
      ...
      notifFail:addNotifFail, notifExpiry:addNotifExpiry,
  ...
  // ── 공통 하단 섹션 렌더 (자동지급/승인통제/증빙/알림) ──
  function renderAutoPaySection(autoOn, setAutoOn, payDay, setPayDay, payMethod, setPayMethod, card, setCard, autoEnd, setAutoEnd, amountVal, cycle, customDay, setCustomDay, bankName, setBankName, bankAccount, setBankAccount) {
            ...
            {limitOn && (
              <div style={{ padding:'0 16px 14px' }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'5px' }}>
                  {[{ id:'block', label:'자동 차단', sub:'한도 초과 즉시 납부 중단' },{ id:'approve', label:'관리자 승인 요청', sub:'초과 전 결재라인 승인 필요' },{ id:'alert', label:'알림만 발송', sub:'납부는 허용, 담당자에게 알림만' }].map(act => (
  ...
  function renderEvidenceSection(evConfirm, setEvConfirm, evReceipt, setEvReceipt) {
    return (
      <>
        {secLabel('증빙 연동')}
        <div style={{ background:COLORS.bgCard, borderRadius:'16px', boxShadow:SHADOWS.card, overflow:'hidden' }}>
          {[
            { icon:'🧾', bg:'#F0FDF4', label:'납부확인서',          sub:'납부 후 납부확인서 자동 수집',    on:evConfirm, toggle:() => setEvConfirm(!evConfirm) },
            { icon:'🏧', bg:'#FFF7ED', label:'이체확인증·카드영수증', sub:'납부 완료 후 자동 수집 및 저장', on:evReceipt, toggle:() => setEvReceipt(!evReceipt) },
          ].map((ev, i, arr) => (
          ...
          <div style={{ padding:'10px 16px', background:COLORS.bgMuted, display:'flex', gap:'8px', alignItems:'center' }}>
            ...
            <div style={{ fontSize:'11px', color:COLORS.t3 }}>저장 위치: <span style={{ fontWeight:700 }}>통합 증빙 센터</span> → 보험료 폴더</div>
          </div>
        </div>
      </>
    )
  ...
  function renderNotifSection(nb, sNb, nd, sNd, nf, sNf, ne, sNe) {
    return (
      <>
        {secLabel('알림 설정')}
        <div style={{ background:COLORS.bgCard, borderRadius:'16px', boxShadow:SHADOWS.card, overflow:'hidden' }}>
          {[
            { icon:'🔔', bg:'#FFF7ED', label:'납부 예정 알림',       sub:'납부 3일 전 사전 안내',      on:nb, toggle:() => sNb(!nb) },
            { icon:'✅', bg:'#F0FDF4', label:'납부 완료 알림',       sub:'납부 완료 즉시 발송',         on:nd, toggle:() => sNd(!nd) },
            { icon:'⚠️', bg:'#FEF2F2', label:'납부 실패 알림',       sub:'실패 즉시 운영자 알림',       on:nf, toggle:() => sNf(!nf) },
            { icon:'📅', bg:'#EFF6FF', label:'계약 만료 30일 전 알림', sub:'갱신 또는 종료 안내',       on:ne, toggle:() => sNe(!ne) },
          ].map((n, i, arr) => (
          ...
          <div style={{ flex:1, overflowY:'auto', background:COLORS.bg }}>
            <div style={{ background:theme.headerGrad, paddingTop:'20px', paddingBottom:'0' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'4px 16px 12px' }}>
                ...
                <button onClick={() => setScreen('log')} style={{ fontSize:'11px', fontWeight:600, color:'rgba(255,255,255,0.85)', background:'rgba(255,255,255,0.14)', border:'1px solid rgba(255,255,255,0.22)', padding:'5px 11px', borderRadius:'20px', cursor:'pointer', fontFamily:'inherit', marginRight:'4px', outline:'none' }}>납부 로그</button>
            ...
            <div style={{ padding:'16px 16px 36px', display:'flex', flexDirection:'column', gap:'6px' }}>
              ...
              {renderAutoPaySection(editAutoOn, setEditAutoOn, editPayDay, setEditPayDay, editPayMethod, setEditPayMethod, editCard, setEditCard, editAutoEnd, setEditAutoEnd, amountVal, editCycle, editCustomDay, setEditCustomDay, editBankName, setEditBankName, editBankAccount, setEditBankAccount)}
              ...
              {renderEvidenceSection(editEvConfirm, setEditEvConfirm, editEvReceipt, setEditEvReceipt)}
              {renderNotifSection(editNotifBefore, setEditNotifBefore, editNotifDone, setEditNotifDone, editNotifFail, setEditNotifFail, editNotifExpiry, setEditNotifExpiry)}
            </div>
          </div>
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          <div style={{ flex:1, overflowY:'auto', background:COLORS.bg }}>
            ...
            <div style={{ padding:'16px 16px 36px', display:'flex', flexDirection:'column', gap:'6px' }}>
              ...
              {renderAutoPaySection(addAutoOn, setAddAutoOn, addPayDay, setAddPayDay, addPayMethod, setAddPayMethod, addCard, setAddCard, addAutoEnd, setAddAutoEnd, amountVal, addCycle, addCustomDay, setAddCustomDay, addBankName, setAddBankName, addBankAccount, setAddBankAccount)}
              ...
              {renderEvidenceSection(addEvConfirm, setAddEvConfirm, addEvReceipt, setAddEvReceipt)}
              {renderNotifSection(addNotifBefore, setAddNotifBefore, addNotifDone, setAddNotifDone, addNotifFail, setAddNotifFail, addNotifExpiry, setAddNotifExpiry)}
            </div>
          </div>
