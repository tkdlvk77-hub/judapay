...
export default function ExecuteSubscription() {
  ...
  const resetShared = () => {
    ...
    setApprovalEnabled(true); setLimitEnabled(false)
    setLimitAmount('500000'); setLimitAction('alert')
    setNotifBefore(true); setNotifDone(true); setNotifFail(true)
    setNotifSurge(true); setNotifLimit(true)
  }
  ...
  const openDetail = (item) => {
    ...
    setSaved(false)
    setApprovalEnabled(item.approvalEnabled !== false)
    setLimitEnabled(item.limitEnabled || false)
    setLimitAmount(String(item.limitAmount || 500000))
    setLimitAction(item.limitAction || 'alert')
    setNotifBefore(item.notifBefore !== false)
    setNotifDone(item.notifDone !== false)
    setNotifFail(item.notifFail !== false)
    setNotifSurge(item.notifSurge !== false)
    setNotifLimit(item.notifLimit !== false)
    setScreen('detail')
  }
    ...
    setItems(prev => prev.map(it =>
      it.id === selectedItem.id
        ? { ...it,
            ...
            limitEnabled, limitAmount:parseInt(limitAmount)||0, limitAction,
            approvalEnabled,
            notifBefore, notifDone, notifFail, notifSurge, notifLimit,
          }
        : it
    ))
    setSaved(true)
    ...
    <>
      ...
      <div style={{ background:COLORS.bgCard, borderRadius:'16px', boxShadow:SHADOWS.card, overflow:'hidden' }}>
        ...
        <div style={{ borderBottom:`1px solid ${COLORS.borderSoft}` }}>
          <div style={{ padding:'13px 16px', display:'flex', alignItems:'center', gap:'12px' }}>
            ...
            <Toggle on={approvalEnabled} onChange={() => setApprovalEnabled(!approvalEnabled)} brand={theme.brand} />
          </div>
          {approvalEnabled && (
        ...
        <div style={{ borderBottom:`1px solid ${COLORS.borderSoft}` }}>
          <div style={{ padding:'13px 16px', display:'flex', alignItems:'center', gap:'12px' }}>
            ...
            <Toggle on={limitEnabled} onChange={() => setLimitEnabled(!limitEnabled)} brand={theme.brand} />
          </div>
          {limitEnabled && (
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'5px' }}>
                {[
                  ...
                  <button key={act.id} onClick={() => setLimitAction(act.id)}
                    style={{ width:'100%', padding:'9px 12px', textAlign:'left', background: limitAction === act.id ? theme.brand+'10' : '#fff', border:`1px solid ${limitAction === act.id ? theme.brand+'50' : COLORS.borderSoft}`, borderRadius:'10px', cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:'8px' }}>
                    <div style={{ width:'14px', height:'14px', borderRadius:'50%', border:`2px solid ${limitAction === act.id ? theme.brand : COLORS.borderSoft}`, background: limitAction === act.id ? theme.brand : '#fff', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      <div style={{ background:COLORS.bgCard, borderRadius:'16px', boxShadow:SHADOWS.card, overflow:'hidden' }}>
        ...
        <div style={{ padding:'10px 16px', background:COLORS.bgMuted, display:'flex', gap:'8px', alignItems:'center' }}>
          ...
          <div style={{ fontSize:'11px', color:COLORS.t3 }}>저장 위치: <span style={{ fontWeight:700 }}>통합 증빙 센터</span> → 구독료 폴더</div>
        </div>
      </div>
      ...
      <div style={{ background:COLORS.bgCard, borderRadius:'16px', boxShadow:SHADOWS.card, overflow:'hidden' }}>
        {[
          { icon:'🔔', bg:'#FFF7ED', label:'결제 예정 알림',   sub:'결제 3일 전 자동 발송',           on:notifBefore, toggle:() => setNotifBefore(!notifBefore) },
          { icon:'✅', bg:'#F0FDF4', label:'결제 완료 알림',   sub:'결제 완료 즉시 담당자 발송',       on:notifDone,   toggle:() => setNotifDone(!notifDone) },
          { icon:'⚠️', bg:'#FEF2F2', label:'결제 실패 알림',   sub:'실패 즉시 운영자·담당자 발송',     on:notifFail,   toggle:() => setNotifFail(!notifFail) },
        ...
        <div style={{ flex:1, overflowY:'auto', background:COLORS.bg }}>
          <div style={{ background:theme.headerGrad, paddingTop:'20px', paddingBottom:'24px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'4px 16px 16px' }}>
              ...
              <span style={{ fontSize:'15px', fontWeight:600, color:'#fff', flex:1 }}>결제 로그</span>
            </div>
            ...
            <div style={{ padding:'16px 16px 36px', display:'flex', flexDirection:'column', gap:'6px' }}>
              ...
              <div style={{ background:COLORS.bgCard, border:`2px solid ${theme.brand}`, borderRadius:'14px', padding:'12px 16px', boxShadow:`0 0 0 3px ${theme.brand}14`, marginBottom:'4px' }}>
                ...
                <div style={{ display:'flex', alignItems:'baseline', gap:'6px', borderBottom:`1.5px solid ${theme.brand}50`, paddingBottom:'6px', marginBottom:'8px' }}>
                  {currency !== 'KRW' && (
                    <span style={{ fontSize:'20px', fontWeight:800, color:COLORS.t3 }}>{CURRENCY_SYMBOLS[currency]}</span>
                  )}
                  ...
                  <span style={{ fontSize:'14px', fontWeight:600, color:COLORS.t3 }}>
  ...
  if (screen === 'detail' && selectedItem) {
    ...
    const isCanceling = computedStatusDetail === 'overdue'
          ...
          <div style={{ flex:1, overflowY:'auto', background:COLORS.bg }}>
            <div style={{ background:theme.headerGrad, paddingTop:'20px', paddingBottom:'0' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'4px 16px 12px' }}>
                ...
                <button onClick={() => setScreen('log')} style={{ fontSize:'11px', fontWeight:600, color:'rgba(255,255,255,0.85)', background:'rgba(255,255,255,0.14)', border:'1px solid rgba(255,255,255,0.22)', padding:'5px 11px', borderRadius:'20px', cursor:'pointer', fontFamily:'inherit', marginRight:'4px' }}>
                  결제 로그
                </button>
                <button onClick={() => setShowExitModal(true)} style={{ width:'32px', height:'32px', background:'rgba(255,255,255,0.15)', border:'none', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', padding:0 }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.85)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
