...
export default function ExecuteTelecom() {
  ...
  const detailBase  = selectedItem ? (parseInt(editAmount) || 0) * editLines : 0
          ...
          <div style={{ flex:1, overflowY:'auto', background:COLORS.bg }}>
            ...
            <div style={{ padding:'16px 16px 36px', display:'flex', flexDirection:'column', gap:'6px' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:'8px', marginTop:'6px', marginBottom:'10px' }}>
                ...
                <span style={{ fontSize:'11px', fontWeight:700, color:theme.brandDark, letterSpacing:'0.6px', textTransform:'uppercase' }}>알림 설정</span>
              </div>
              <div style={{ background:COLORS.bgCard, borderRadius:'16px', boxShadow:SHADOWS.card, padding:'16px', display:'flex', flexDirection:'column', gap:'14px', marginBottom:'4px' }}>
                {[
                  { label:'지급 전 알림',  sub:'지급 1일 전 사전 안내', val:notifBefore, set:setNotifBefore },
                  { label:'지급 완료 알림', sub:'납부 완료 즉시 발송',   val:notifDone,   set:setNotifDone },
                  { label:'지급 실패 알림', sub:'실패 즉시 관리자 알림',  val:notifFail,   set:setNotifFail },
                ].map((row, i) => (
                  <div key={i}>
              ...
              <div style={{ padding:'12px 14px', background:`${theme.brandDark}0A`, border:`1px solid ${theme.brandDark}15`, borderRadius:'12px', fontSize:'11px', color:theme.brandDark, lineHeight:1.7, display:'flex', gap:'8px', alignItems:'flex-start', marginTop:'6px' }}>
                <span style={{ flexShrink:0, marginTop:'1px' }}>ⓘ</span>
                <span>통신비는 매월 자동 지급됩니다. 세금계산서·이체확인증이 자동 수집됩니다.</span>
              </div>
            </div>
          </div>
      ...
      <div style={{ flex:1, overflowY:'auto', background:COLORS.bg }}>
        ...
        <div style={{ padding:'16px 16px 36px' }}>
          <div style={{ display:'flex', flexDirection:'column', gap:'8px', marginBottom:'16px' }}>
            {items.map(item => {
              const { total, vat } = calcTotal(item)
