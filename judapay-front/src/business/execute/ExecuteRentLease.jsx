...
export default function ExecuteRentLease() {
  ...
  // 승인및통제 + 증빙 + 알림 (공통 하단 섹션 — 발주정보는 링크수취형 선택 시 SectionControl 내부에만 표시)
  const renderCommonBottomSections = () => (
    <>
      ...
      <div style={{ background: COLORS.bgCard, borderRadius:'16px', boxShadow: SHADOWS.card, overflow:'hidden' }}>
        {[
          { icon:'✅', bg:'#F0FDF4', label:'지급 전 승인 필요', sub:'대표이사 또는 CFO 승인 후 집행' },
          { icon:'🔔', bg:'#FFF7ED', label:'잔액 부족 시 알림', sub:'지급 3일 전 잔액 확인 후 부족 시 발송' },
          { icon:'🔁', bg:'#EFF6FF', label:'지급 실패 시 재시도', sub:'6시간 간격 3회 재시도 후 최종 실패 알림' },
        ].map((row, i, arr) => (
          <div key={row.label} style={{ padding:'13px 16px', display:'flex', alignItems:'center', gap:'12px', borderBottom: i < arr.length-1 ? `1px solid ${COLORS.borderSoft}` : 'none' }}>
            <div style={{ width:'36px', height:'36px', borderRadius:'12px', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'16px', background: row.bg }}>{row.icon}</div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:'13px', fontWeight:600, color: COLORS.t1, marginBottom:'2px' }}>{row.label}</div>
              <div style={{ fontSize:'11px', color: COLORS.t4, lineHeight:1.4 }}>{row.sub}</div>
            </div>
      ...
      <div style={{ background: COLORS.bgCard, borderRadius:'16px', boxShadow: SHADOWS.card, overflow:'hidden' }}>
        {[
          { icon:'📄', bg:'#EFF6FF', label:'렌트·리스 계약서', sub:'계약서 첨부 시 증빙 센터 자동 보관', on: hasContractFile, toggle: () => setHasContractFile(!hasContractFile) },
          { icon:'🧾', bg:'#F0FDF4', label:'세금계산서', sub:'납부 후 세금계산서 자동 수집', on: hasTax, toggle: () => setHasTax(!hasTax) },
          { icon:'🏧', bg:'#FFF7ED', label:'이체확인증 · 카드영수증', sub:'이체 완료 후 자동 발행 및 저장', on: hasReceipt, toggle: () => setHasReceipt(!hasReceipt) },
        ].map((ev, i, arr) => (
        ...
        <div style={{ padding:'10px 16px', background: COLORS.bgMuted, display:'flex', gap:'8px', alignItems:'center' }}>
          ...
          <div style={{ fontSize:'11px', color: COLORS.t3 }}>저장 위치: <span style={{ fontWeight:700 }}>통합 증빙 센터</span> → 렌트&리스 폴더</div>
        </div>
      </div>

      {secLabel('알림 설정')}
      <div style={{ background: COLORS.bgCard, borderRadius:'16px', boxShadow: SHADOWS.card, padding:'16px', display:'flex', flexDirection:'column', gap:'14px' }}>
        {[
          { icon:'🔔', label:'지급 예정 알림',         sub:'지급 3일 전 사전 안내',     val:notifBefore, set:setNotifBefore },
          { icon:'✅', label:'지급 완료 알림',         sub:'납부 완료 즉시 발송',       val:notifDone,   set:setNotifDone },
          { icon:'⚠️', label:'지급 실패 알림',         sub:'실패 즉시 운영자 알림',     val:notifFail,   set:setNotifFail },
          { icon:'📅', label:'계약 만료 30일 전 알림', sub:'갱신 또는 종료 안내',       val:notifExpiry, set:setNotifExpiry },
          { icon:'🔄', label:'갱신 필요 알림',         sub:'계약 만료 7일 전 발송',     val:notifRenew,  set:setNotifRenew },
        ].map((row, i) => (
          <div key={i}>
            {i > 0 && <div style={{ height:'1px', background:COLORS.borderSoft, marginBottom:'14px' }} />}
            <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:'12px' }}>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:'13px', fontWeight:700, color:COLORS.t1, marginBottom:'2px' }}>{row.icon} {row.label}</div>
                <div style={{ fontSize:'11px', color:COLORS.t4 }}>{row.sub}</div>
              </div>
              <Toggle on={row.val} onChange={() => row.set(!row.val)} brand={theme.brand} />
            </div>
          </div>
        ))}
      </div>
          ...
          <div style={{ flex:1, overflowY:'auto', background: COLORS.bg }}>
            ...
            <div style={{ padding:'16px 16px 36px', display:'flex', flexDirection:'column', gap:'6px' }}>
              ...
              <div style={{ background: COLORS.bgCard, border:`2px solid ${theme.brand}`, borderRadius:'14px', padding:'12px 16px', boxShadow:`0 0 0 3px ${theme.brand}14`, marginBottom:'4px' }}>
                ...
                <div style={{ display:'flex', alignItems:'baseline', gap:'6px', borderBottom:`1.5px solid ${theme.brand}50`, paddingBottom:'6px', marginBottom:'4px' }}>
                  <input type="number" value={editAmount} onChange={e => setEditAmount(e.target.value)}
                    style={{ flex:1, border:'none', outline:'none', fontSize:'22px', fontWeight:800, color: COLORS.t1, background:'transparent', fontFamily:'inherit', letterSpacing:'-0.5px', padding:0 }}/>
                ...
                {[
                  ...
                  <div key={f.label} style={{ padding:'13px 16px', borderBottom: i < arr.length-1 ? `1px solid ${COLORS.borderSoft}` : 'none', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    ...
                    <input type="date" value={f.value} onChange={e => f.setter(e.target.value)}
                      style={{ border:'none', outline:'none', fontSize:'13px', fontWeight:600, color: COLORS.t1, background:'transparent', fontFamily:'inherit' }}/>
                  </div>
                ))}
              </div>
