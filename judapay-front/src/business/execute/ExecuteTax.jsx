...
export default function ExecuteTax() {
  ...
  const [detailNotifBefore, setDetailNotifBefore] = useState(true)
  const [detailNotifDone,   setDetailNotifDone]   = useState(true)
  const [detailNotifFail,   setDetailNotifFail]   = useState(true)
  ...
  function openDetail(n, from) {
    ...
    setDetailNotifBefore(n.notifBefore)
    setDetailNotifDone(n.notifDone)
    setDetailNotifFail(n.notifFail)
    setSaved(false)
        ...
        <div style={{ flex:1, overflowY:'auto', background:COLORS.bg }}>
          ...
          <div style={{ padding:'16px 16px 32px', display:'flex', flexDirection:'column', gap:'10px' }}>
            {paidList.length === 0 ? (
              ...
              return (
                <div key={n.id} onClick={() => openDetail(n, 'paid')}
      ...
      <PhoneShell>
        <div style={{ flex:1, overflowY:'auto', background:COLORS.bg }}>
          ...
          <div style={{ padding:'20px 16px 32px', display:'flex', flexDirection:'column', gap:'16px' }}>
            ...
            <div style={{ background:'#fff', borderRadius:'16px', padding:'16px 18px', boxShadow:SHADOWS.card }}>
              <SecLabel brand={theme.brand} brandDark={theme.brandDark}>알림 설정</SecLabel>
              {[
                { label:'납부 3일 전 알림', sub:'납부기한 3일 전에 미리 알려드립니다', val:detailNotifBefore, set:setDetailNotifBefore },
                { label:'납부 완료 알림',    sub:'자동 납부 완료 시 알림을 받습니다',   val:detailNotifDone,   set:setDetailNotifDone   },
                { label:'납부 실패 알림',    sub:'납부 실패 시 즉시 알림을 받습니다',   val:detailNotifFail,   set:setDetailNotifFail   },
              ].map((row, i, arr) => (
                <div key={row.label} style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
                  ...
                  <div>
                    <div style={{ fontSize:'13px', fontWeight:600, color:COLORS.t1 }}>{row.label}</div>
                    <div style={{ fontSize:'11px', color:COLORS.t3, marginTop:'2px' }}>{row.sub}</div>
                  </div>
                  <Toggle on={row.val} onChange={() => row.set(v=>!v)} brand={theme.brand} />
                </div>
              ))}
            </div>
            {!sel.paid && (
              <button onClick={handleSave}
                style={{ width:'100%', height:'52px', background: saved ? '#10B981' : theme.brand,
                  color:'#fff', border:'none', outline:'none', borderRadius:'14px',
                  fontSize:'15px', fontWeight:700, cursor:'pointer', fontFamily:'inherit',
                  transition:'background 0.3s', boxShadow:SHADOWS.buttonBrand }}>
                {saved ? '✓ 저장되었습니다' : '설정 저장'}
              </button>
            )}
          </div>
