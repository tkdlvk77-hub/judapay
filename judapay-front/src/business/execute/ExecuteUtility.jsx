...
// ─── 승인·통제·증빙·알림 공통 섹션 ──────────────────────
function CommonBottomSections({ theme, approvalEnabled, setApprovalEnabled, limitEnabled, setLimitEnabled, limitAmount, setLimitAmount, limitAction, setLimitAction, hasTax, setHasTax, hasReceipt, setHasReceipt, notifBefore, setNotifBefore, notifDone, setNotifDone, notifFail, setNotifFail, notifSurge, setNotifSurge, notifLimit, setNotifLimit }) {
  const brand = theme.brand

  return (
    <>
      {/* 승인 및 통제 */}
      <SecLabel label="승인 및 통제" brand={brand} />
      <div style={{ background:COLORS.bgCard, borderRadius:'16px', boxShadow:SHADOWS.card, overflow:'hidden' }}>
        ...
        <div style={{ borderBottom:`1px solid ${COLORS.borderSoft}` }}>
          <div style={{ padding:'13px 16px', display:'flex', alignItems:'center', gap:'12px' }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:'11px', color:COLORS.t4, lineHeight:1.4 }}>관리자관리 → 결재라인 설정 기준 자동 적용</div>
            </div>
            <Toggle on={approvalEnabled} onChange={() => setApprovalEnabled(!approvalEnabled)} brand={brand} />
          </div>
          {approvalEnabled && (
            <div style={{ padding:'8px 16px 12px', background:`${brand}06` }}>
          ...
          <div style={{ padding:'13px 16px', display:'flex', alignItems:'center', gap:'12px' }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:'11px', color:COLORS.t4 }}>
                {limitEnabled
                  ? `${fmt(parseInt(limitAmount)||0)}원 초과 시 ${limitAction==='block'?'자동 차단':limitAction==='approve'?'승인 요청':'알림 발송'}`
                  : '항목별 월 지출 한도를 설정합니다'}
              </div>
            </div>
            <Toggle on={limitEnabled} onChange={() => setLimitEnabled(!limitEnabled)} brand={brand} />
          </div>
          {limitEnabled && (
            <div style={{ padding:'0 16px 14px' }}>
              <div style={{ background:COLORS.bgMuted, borderRadius:'12px', padding:'12px 14px', marginBottom:'10px' }}>
                ...
                <div style={{ display:'flex', alignItems:'baseline', gap:'6px' }}>
                  <input type="number" value={limitAmount} onChange={e => setLimitAmount(e.target.value)}
                    style={{ flex:1, border:'none', outline:'none', fontSize:'20px', fontWeight:800, color:brand, background:'transparent', fontFamily:'inherit', letterSpacing:'-0.5px' }}/>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'5px' }}>
                {[
                  { id:'block',   label:'자동 차단',       sub:'한도 초과 즉시 결제 중단' },
                  { id:'approve', label:'관리자 승인 요청', sub:'초과 결제 전 결재라인 승인 필요' },
                  { id:'alert',   label:'알림만 발송',      sub:'결제는 허용, 담당자에게 알림만 발송' },
                ].map(act => (
                  <button key={act.id} onClick={() => setLimitAction(act.id)}
                    style={{ width:'100%', padding:'9px 12px', textAlign:'left', background: limitAction === act.id ? `${brand}10` : '#fff', border:`1px solid ${limitAction === act.id ? `${brand}50` : COLORS.borderSoft}`, borderRadius:'10px', cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:'8px' }}>
                    <div style={{ width:'14px', height:'14px', borderRadius:'50%', border:`2px solid ${limitAction === act.id ? brand : COLORS.borderSoft}`, background: limitAction === act.id ? brand : '#fff', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
                      {limitAction === act.id && <div style={{ width:'5px', height:'5px', borderRadius:'50%', background:'#fff' }}/>}
                    </div>
                    <div>
        ...
        <div style={{ padding:'13px 16px', display:'flex', alignItems:'center', gap:'12px', borderBottom:`1px solid ${COLORS.borderSoft}` }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:'11px', color:COLORS.t4, lineHeight:1.4 }}>지급 3일 전 잔액 확인 후 부족 시 발송</div>
          </div>
          <div style={{ fontSize:'10px', fontWeight:700, padding:'3px 9px', background:`${brand}15`, color:theme.brandDark, borderRadius:'20px', flexShrink:0 }}>기본</div>
        </div>
        <div style={{ padding:'13px 16px', display:'flex', alignItems:'center', gap:'12px' }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:'11px', color:COLORS.t4, lineHeight:1.4 }}>6시간 간격 3회 재시도 후 최종 실패 알림</div>
          </div>
          <div style={{ fontSize:'10px', fontWeight:700, padding:'3px 9px', background:`${brand}15`, color:theme.brandDark, borderRadius:'20px', flexShrink:0 }}>기본</div>
        </div>
      </div>
      ...
      {/* 증빙 연동 */}
      <SecLabel label="증빙 연동" brand={brand} />
      <div style={{ background:COLORS.bgCard, borderRadius:'16px', boxShadow:SHADOWS.card, overflow:'hidden' }}>
        {[
          { icon:'🧾', bg:'#F0FDF4', label:'세금계산서',             sub:'납부 후 세금계산서 자동 수집',    on:hasTax,     toggle:() => setHasTax(!hasTax) },
          { icon:'🏧', bg:'#FFF7ED', label:'카드영수증 · 이체확인증', sub:'결제 완료 후 자동 수집 및 저장', on:hasReceipt, toggle:() => setHasReceipt(!hasReceipt) },
        ].map((ev, i, arr) => (
          <div key={ev.label} style={{ padding:'13px 16px', display:'flex', alignItems:'center', gap:'12px', borderBottom: i < arr.length-1 ? `1px solid ${COLORS.borderSoft}` : 'none' }}>
            ...
            <Toggle on={ev.on} onChange={ev.toggle} brand={brand} />
          </div>
        ))}
        <div style={{ padding:'10px 16px', background:COLORS.bgMuted, display:'flex', gap:'8px', alignItems:'center' }}>
          <span style={{ fontSize:'13px' }}>📁</span>
          <div style={{ fontSize:'11px', color:COLORS.t3 }}>저장 위치: <span style={{ fontWeight:700 }}>통합 증빙 센터</span> → 공과금 폴더</div>
        </div>
      </div>

      {/* 알림 설정 */}
      <SecLabel label="알림 설정" brand={brand} />
      <div style={{ background:COLORS.bgCard, borderRadius:'16px', boxShadow:SHADOWS.card, overflow:'hidden' }}>
        {[
          { icon:'🔔', bg:'#FFF7ED', label:'납부 예정 알림',   sub:'납부 3일 전 자동 발송',           on:notifBefore, toggle:() => setNotifBefore(!notifBefore) },
          { icon:'✅', bg:'#F0FDF4', label:'납부 완료 알림',   sub:'납부 완료 즉시 담당자 발송',       on:notifDone,   toggle:() => setNotifDone(!notifDone) },
          { icon:'⚠️', bg:'#FEF2F2', label:'납부 실패 알림',   sub:'실패 즉시 운영자·담당자 발송',     on:notifFail,   toggle:() => setNotifFail(!notifFail) },
          { icon:'📈', bg:'#FFF7ED', label:'금액 급증 감지',   sub:'전월 대비 30% 이상 증가 시 알림',  on:notifSurge,  toggle:() => setNotifSurge(!notifSurge) },
          { icon:'🚨', bg:'#FEF2F2', label:'월 한도 초과 알림', sub:'설정 한도 초과 시 즉시 발송',     on:notifLimit,  toggle:() => setNotifLimit(!notifLimit) },
        ].map((n, i, arr) => (
          <div key={n.label} style={{ padding:'13px 16px', display:'flex', alignItems:'center', gap:'12px', borderBottom: i < arr.length-1 ? `1px solid ${COLORS.borderSoft}` : 'none' }}>
            ...
            <Toggle on={n.on} onChange={n.toggle} brand={brand} />
          </div>
        ))}
      </div>
    </>
...
export default function ExecuteUtility() {
  ...
  const [limitAmount, setLimitAmount]     = useState('500000')
  const [limitAction, setLimitAction]     = useState('alert')
  ...
  const [notifSurge, setNotifSurge]       = useState(true)
  ...
  const resetShared = () => {
    ...
    setApprovalEnabled(false); setLimitEnabled(false); setLimitAmount('500000'); setLimitAction('alert')
    setHasTax(false); setHasReceipt(true)
    setNotifBefore(true); setNotifDone(true); setNotifFail(true); setNotifSurge(true); setNotifLimit(true)
    setSaved(false)
  }
  ...
  const openDetail = (item) => {
    ...
    setApprovalEnabled(item.approvalEnabled); setLimitEnabled(item.limitEnabled)
    setLimitAmount(String(item.limitAmount)); setLimitAction(item.limitAction)
    setHasTax(item.hasTax); setHasReceipt(item.hasReceipt)
    setNotifBefore(item.notifBefore); setNotifDone(item.notifDone); setNotifFail(item.notifFail)
    setNotifSurge(item.notifSurge); setNotifLimit(item.notifLimit)
    setSaved(false)
    setScreen('detail')
  }
    ...
    setItems(prev => prev.map(it =>
      it.id === selectedItem.id
        ? { ...it, name:editName, supplier:editSupplier, customerNo:editCustomerNo,
            ...
            approvalEnabled, limitEnabled, limitAmount:parseInt(limitAmount)||0, limitAction,
  ...
  const commonBottomProps = {
    theme, approvalEnabled, setApprovalEnabled, limitEnabled, setLimitEnabled,
    limitAmount, setLimitAmount, limitAction, setLimitAction,
    hasTax, setHasTax, hasReceipt, setHasReceipt,
    notifBefore, setNotifBefore, notifDone, setNotifDone,
    notifFail, setNotifFail, notifSurge, setNotifSurge, notifLimit, setNotifLimit,
  }

  // ═══════════════════════════════════════════════════════════
  // ── 납부 로그 화면
  // ═══════════════════════════════════════════════════════════
  if (screen === 'log' && selectedItem) {
        ...
        <div style={{ flex:1, overflowY:'auto', background:COLORS.bg }}>
          <div style={{ background:theme.headerGrad, paddingTop:'20px', paddingBottom:'24px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'4px 16px 16px' }}>
              <button onClick={() => setScreen('detail')} style={{ width:'32px', height:'32px', background:'rgba(255,255,255,0.15)', border:'none', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', padding:0 }}>
              ...
              <span style={{ fontSize:'15px', fontWeight:600, color:'#fff', flex:1 }}>납부 로그</span>
            </div>
        ...
        <div style={{ flex:1, overflowY:'auto', background:COLORS.bg }}>
          <div style={{ background:theme.headerGrad, paddingTop:'20px', paddingBottom:'0' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'4px 16px 12px' }}>
              <button onClick={() => setScreen('list')} style={{ width:'32px', height:'32px', background:'rgba(255,255,255,0.15)', border:'none', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', padding:0 }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
              </button>
      ...
      <PhoneShell>
        <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          <div style={{ flex:1, overflowY:'auto', background:COLORS.bg }}>
            <div style={{ background:theme.headerGrad, paddingTop:'20px', paddingBottom:'0' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'4px 16px 12px' }}>
                <button onClick={() => setScreen('addType')} style={{ width:'32px', height:'32px', background:'rgba(255,255,255,0.15)', border:'none', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', padding:0 }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
                </button>
                <span style={{ fontSize:'15px', fontWeight:600, color:'#fff', flex:1 }}>{addType.icon} {addType.label} 등록</span>
                <button onClick={() => setShowExitModal(true)} style={{ width:'32px', height:'32px', background:'rgba(255,255,255,0.15)', border:'none', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', padding:0 }}>
            ...
            <div style={{ padding:'16px 16px 36px', display:'flex', flexDirection:'column', gap:'6px' }}>
              ...
              <CommonBottomSections {...commonBottomProps} />
      ...
      <div style={{ flex:1, overflowY:'auto', background:COLORS.bg }}>
        ...
        <div style={{ background:theme.headerGrad, paddingTop:'20px', paddingBottom:'20px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'4px 16px 14px' }}>
            ...
            <div style={{ flex:1 }}/>
            <button onClick={() => setShowExitModal(true)} style={{ width:'32px', height:'32px', background:'transparent', border:'none', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', padding:0 }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.7)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>
          </div>
