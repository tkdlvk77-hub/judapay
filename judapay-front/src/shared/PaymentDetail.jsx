...
export default function PaymentDetail() {
  ...
  return (
    <PhoneShell>
      ...
      {/* ── MCC 설정 바텀시트 ── */}
      {showMCC && (
        <div style={{ position:'absolute', inset:0, background:'rgba(0,0,0,0.5)', zIndex:200,
          display:'flex', flexDirection:'column', justifyContent:'flex-end' }}
          onClick={() => setShowMCC(false)}>
          <div style={{ background:'#fff', borderRadius:`${RADIUS.lg} ${RADIUS.lg} 0 0`,
            padding:'20px 16px 32px', maxHeight:'80%', overflowY:'auto' }}
            onClick={e => e.stopPropagation()}>
            {/* 핸들 */}
            <div style={{ width:'36px', height:'4px', background: COLORS.border,
              borderRadius:'2px', margin:'0 auto 18px' }} />
            <div style={{ fontSize:'16px', fontWeight:700, color: COLORS.t1, marginBottom:'4px' }}>
              MCC 설정
            </div>
            <div style={{ fontSize:'12px', color: COLORS.t4, marginBottom:'16px' }}>
              카드 결제 시 차단할 업종을 설정하세요.
            </div>
            <MccBlock items={mccItems} onChange={setMccItems} showInfoBox={false} />
            {/* 버튼 */}
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px', marginTop:'16px' }}>
              <button onClick={() => setShowMCC(false)}
                style={{ height:'48px', background: COLORS.bgMuted, color: COLORS.t2,
                  border:'none', borderRadius: RADIUS.md, fontSize:'14px', fontWeight:600,
                  cursor:'pointer', fontFamily:'inherit' }}>
                취소
              </button>
              <button onClick={() => setShowMCC(false)}
                style={{ height:'48px', background: theme.activeBtnGrad || theme.headerGrad,
