...
function Step1({ onSelect }) {
  return (
    <>
      <Header onBack={() => history.back()} step={1} totalSteps={4}
...
function Step2({ type, onBack, onNext }) {
  ...
  return (
    <>
      ...
      <div style={{ padding:'12px 16px 24px', borderTop:`1px solid ${COLORS.borderSoft}`, background: COLORS.bgCard }}>
...
function Step3({ type, txn, onBack, onNext }) {
  ...
  const valid = checked.length > 0 && desc.trim().length >= 10
      ...
      <div style={{ padding:'20px 16px 32px' }}>
        ...
        <div style={{ marginBottom:'18px' }}>
          ...
          <div style={{ textAlign:'right', fontSize:'10px', color: COLORS.t4, marginTop:'4px', paddingRight:'4px' }}>
            {desc.length}자 {desc.length < 10 && <span style={{ color: COLORS.danger }}>(최소 10자)</span>}
          </div>
        </div>
...
function Step4({ type, txn, content, onBack, onSubmit }) {
  ...
  return (
    <>
      ...
      <div style={{ padding:'12px 16px 24px', borderTop:`1px solid ${COLORS.borderSoft}`, background: COLORS.bgCard, display:'flex', flexDirection:'column', gap:'8px' }}>
      ...
      <div ref={scrollRef} style={{ flex:1, overflowY:'auto', background: COLORS.bg }}>
        ...
        {step === 2 && (
          <Step2
            type={selectedType}
            onBack={() => setStep(1)}
            onNext={(txn) => { setSelectedTxn(txn); setStep(3) }}
          />
        )}
        {step === 3 && (
          <Step3
            ...
            txn={selectedTxn}
            onBack={() => setStep(2)}
            onNext={(c) => { setContent(c); setStep(4) }}
          />
        )}
        {step === 4 && (
          <Step4
